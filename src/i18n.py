...
T = {
    ...
    "lbl_pending_hint":  {"en": "Click OUT (or press O) at the end point to finish", "ja": "終点で OUT をクリック（または O キー）で確定"},
    "lbl_pending_hint_in": {"en": "Click IN (or press I) at the start point to finish", "ja": "始点で IN をクリック（または I キー）で確定"},
    "lbl_pending_hint_start": {"en": "Click IN (or press I) to start a clip at the playhead",
                               "ja": "IN をクリック（または I キー）で再生位置から切り抜き開始"},
    "tip_pending_in":    {"en": "Click: set IN at the playhead (or press I / Ctrl-click the timeline)",
                          "ja": "クリックで再生位置（赤い線）に IN を設定（I キー / タイムラインを Ctrl+クリックでも可）"},
    "tip_pending_out":   {"en": "Click: set OUT at the playhead (or press O / Alt-click the timeline)",
                          "ja": "クリックで再生位置（赤い線）に OUT を設定（O キー / タイムラインを Alt+クリックでも可）"},
